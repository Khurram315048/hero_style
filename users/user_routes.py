...
def user_login():
    if request.method=='POST':
        try:
            data=UserLoginValidator(
                email=request.form.get('email', ''),
                password=request.form.get('password', '')
            )
        except ValidationError as e:
            for msg in extract_errors(e):
                flash(msg, 'danger')
...
def reset_password():
    ...
    if request.method=='POST' and request.form.get('step')=='send_otp':
        try:
            data=OTPEmailValidator(email=request.form.get('email', ''))
        ...
        sent=send_reset_otp(data.email,mail)
        if not sent:
            session['toast']='Email not found!'
            return redirect(url_for('users.user_signup'))
...
def verify_otp():
    ...
    if request.method=='POST' and request.form.get('step')=='verify':
        try:
            data=OTPVerifyValidator(otp=request.form.get('otp',''))
        except ValidationError as e:
            session['toast']=extract_errors(e)[0]
            return redirect(url_for('users.verify_otp'))

        if not verify_reset_otp(email,data.otp):
            session['toast']='Invalid or expired OTP!'
            return redirect(url_for('users.verify_otp'))
...
def set_new_password():
    ...
    if request.method=='POST':
        ...
        result=update_password(email,data.new_password,data.confirm_password)
        if result in ('mismatch','too_short'):
            session['toast']='Password update failed. Please try again.'
            return redirect(url_for('users.set_new_password'))
...
def user_profile():
    ...
    if request.method=='POST':
        ...
        update_user_profile(user_id,data.first_name,data.last_name,data.email)
        session['toast']='Profile Updated Successfully!'
        return redirect(url_for('users.user_profile'))
