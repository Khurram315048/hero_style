...
def place_order():
    ...
    try:
        ...
        for item in cart_items:
            cursor.execute("""SELECT stock_quantity,title FROM products 
        ...
        for item in cart_items:
            ...
            cursor.execute("""UPDATE products SET status='draft'
...
def buy_now(product_id):
    ...
    try:
        ...
        if old_cart_id:
            cursor.execute("DELETE FROM cart_items WHERE cart_id=%s",(old_cart_id,))
            cursor.execute("DELETE FROM carts WHERE cart_id=%s",(old_cart_id,))
            mysql.connection.commit()
        ...
        cursor.execute("""INSERT INTO cart_items(cart_id,product_id,quantity,price_at_add)
            VALUES (%s,%s,%s,%s) ON DUPLICATE KEY UPDATE quantity=VALUES(quantity)
            """,(cart_id,product_id,quantity,price))
        mysql.connection.commit()
